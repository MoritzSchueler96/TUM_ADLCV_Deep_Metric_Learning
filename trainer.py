...
class Trainer:
    def __init__(self, config, save_folder_nets, save_folder_results, device, timer):
        ...
        self.timer = timer
        self.fn = self.config["dataset"]["dataset_short"] + "_intermediate_model_" + str(timer)
    ...
    def train(self):
        ...
        for i in range(self.num_iter):
            ...
            self.evaluator = Evaluator_DML(nb_clusters=self.nb_clusters, dev=self.device, **self.config["eval_params"])
        ...
        for e in range(1, train_params["num_epochs"] + 1):
            ...
            # If not testing
                ...
                if e == 31:
                    ...
                    self.gnn.load_state_dict(torch.load(osp.join(self.save_folder_nets, "gnn_" + self.fn + ".pth")))
    ...
    def forward_pass(self, x, Y, I, P, train_params):
        ...
        if self.gnn_loss or self.of:
            edge_attr, edge_index, fc7, adj_mat = self.graph_generator.get_graph(fc7, Y)
            if type(loss) != int:
    ...
    def evaluate(self, eval_params, scores, e, best_recall_iter):
        if not self.config["mode"] == "pretraining":
            ...
            logger.info("Loss {}, Accuracy {}".format(torch.mean(loss.cpu()), self.running_corrects / self.denom))
    ...
    def save_results(self, train_params, since, end, best_recall_iter, scores):
        ...
        if not self.config["mode"] == "pretraining":
            with open(osp.join(results_dir, file_name + ".txt"), "w") as fp:
                fp.write(file_name + "\n")
    ...
    def get_loss_fn(self, params, num_classes):
        ...
        if not self.every:  # normal GNN loss
            ...
            if "gnn" in params["fns"].split("_"):
                ...
                self.gnn_loss = [
                    losses.CrossEntropyLabelSmooth(num_classes=num_classes, dev=self.gnn_dev).to(self.device)
                    for _ in range(self.config["models"]["gnn_params"]["gnn"]["num_layers"])
                ]

        # CrossEntropy Loss
        if "lsce" in params["fns"].split("_"):
            self.ce = losses.CrossEntropyLabelSmooth(num_classes=num_classes, dev=self.device).to(self.device)
        ...
        if "center" in params["fns"].split("_"):
            ...
            self.opt_center = torch.optim.SGD(self.center.parameters(), lr=0.5)
        else:
            self.center = None
