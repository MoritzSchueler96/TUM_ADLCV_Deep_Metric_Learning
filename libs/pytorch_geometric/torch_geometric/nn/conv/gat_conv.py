...
class GATConv(MessagePassing):
    ...
    def message(self, x_j: Tensor, alpha_j: Tensor, alpha_i: OptTensor,
        ...
        alpha = deterministic_softmax(alpha, index, size_i)

        """
        alpha = softmax(alpha, index, ptr, size_i)
        if(not torch.allclose(alp, alpha)):
            raise NotImplementedError
        """
...
def deterministic_softmax(src, index, batch_size):
    ...
    dev = [src.get_device() if src.get_device() >=0 else "cpu"][0]
    src = src.t()
    src2 = rearrange(src, "h (n a)-> h n a", n=batch_size)
    src_max = torch.max(src2, dim=-2).values

    # exp(src - max_node)
    head_indices = torch.arange(src.shape[0]).type(torch.LongTensor).view(src.shape[0], 1).expand(-1, index.shape[0]).to(dev)
