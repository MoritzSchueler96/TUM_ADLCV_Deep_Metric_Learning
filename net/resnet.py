...
class ResNet(nn.Module):
    def __init__(
        ...
        if self.neck:
            ...
            self.fc = nn.Linear(int((512 * block.expansion) / red), num_classes, bias=False)
...
def _resnet(arch, block, layers, pretrained, progress, last_stride=0, neck=0, red=1, **kwargs):
    model = ResNet(block, layers, last_stride=last_stride, neck=neck, red=red, **kwargs)
    if pretrained:
        if not neck:
...
def resnet18(pretrained=False, progress=True, last_stride=0, neck=0, red=1, **kwargs):
    ...
    """
    return _resnet("resnet18", BasicBlock, [2, 2, 2, 2], pretrained, progress, last_stride, neck, red=red, **kwargs)
...
def resnet50(pretrained=False, progress=True, last_stride=0, neck=0, red=1, **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet("resnet50", Bottleneck, [3, 4, 6, 3], pretrained, progress, last_stride, neck, red=red, **kwargs)
